        ...
        <div className="hero-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "center" }}>
          <div>
            ...
            <div style={{ display: "flex", gap: 16, marginTop: 32, flexWrap: "wrap" }}>
              <Button variant="light" onClick={() => onNav("/our-business")}>Product</Button>
          ...
          <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "grid", gap: 12 }}>
            {bullets.map((b, i) => (
              <li key={i} style={{ display: "flex", gap: 12, fontSize: "var(--text-medium)" }}>
                <Icon name="check" size={22} color="var(--color-deep-sea-green)" />
                {b}
              </li>
            ))}
          </ul>
...
function ProcessTimeline() {
  ...
  return (
    <section className="scheme-3" style={{ ...SECTION, overflow: "hidden" }}>
      <div style={WRAP}>
      ...
      <div style={WRAP}>
        ...
        <div className="methods" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 32 }}>
          {methods.map(([ic, h, d, link], i) => (
            <div key={i}>
              <Icon name={ic} size={44} color="var(--color-deep-sea-green)" />
              <h4 style={{ margin: "20px 0 12px", fontWeight: 700 }}>{h}</h4>
              <p style={{ marginBottom: 16, fontSize: "var(--text-regular)" }}>{d}</p>
              <a href="#" className="wstk-link" style={{ fontSize: "var(--text-regular)" }}>{link}</a>
            </div>
          ))}
        </div>
      </div>
